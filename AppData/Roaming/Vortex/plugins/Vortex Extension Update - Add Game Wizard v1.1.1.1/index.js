...
const react_select_1 = __webpack_require__(/*! react-select */ "./node_modules/react-select/dist/react-select.es.js");
...
function FormLine(props) {
    ...
    const { state, reason } = (validationFunc === null || validationFunc === void 0 ? void 0 : validationFunc()) || {};
    return (React.createElement(react_bootstrap_1.FormGroup, { controlId: controlId, className: "form-line", validationState: state },
        React.createElement(react_bootstrap_1.Col, { sm: 3 },
            React.createElement(react_bootstrap_1.ControlLabel, null, title)),
        React.createElement(react_bootstrap_1.Col, { sm: 9 }, children),
        reason !== undefined ? React.createElement(react_bootstrap_1.HelpBlock, null, t(reason)) : null));
}
function Suggestion(props) {
    return (React.createElement("div", null,
        React.createElement("a", { onClick: () => props.onChange(props.value) }, props.value)));
}
// mostly copy&paste from Vortex core because it's not exported atm
function FormPathItem(props) {
    const { t, label, value, onChange } = props;
    const onChangeEvt = React.useCallback((evt) => {
    ...
    return (React.createElement(react_bootstrap_1.FormGroup, null,
        React.createElement(react_bootstrap_1.Col, { sm: 3, className: 'ag-form-label' }, label),
        React.createElement(react_bootstrap_1.Col, { sm: 9, className: 'ag-form-value' },
            React.createElement(react_bootstrap_1.InputGroup, null,
                React.createElement(react_bootstrap_1.FormControl, { type: 'text', value: value, onChange: onChangeEvt }),
                React.createElement(react_bootstrap_1.InputGroup.Button, { className: 'inset-btn' },
                    React.createElement(vortex_api_1.tooltip.IconButton, { id: 'change-tool-path', tooltip: t('Change'), onClick: onChangeEdit, icon: 'browse' }))))));
}
function PathInput(props) {
    const browseCB = React.useCallback(() => {
        ...
        prom.then((selectedPath) => {
            if (selectedPath) {
                props.onChange(selectedPath);
            }
        });
    }, [props.onChange, props.defaultpath, props.selectFile]);
    const changeCB = React.useCallback((evt) => {
        props.onChange(evt.currentTarget.value);
    }, [props.onChange]);
    return (React.createElement(react_bootstrap_1.InputGroup, null,
        React.createElement(react_bootstrap_1.FormControl, { className: 'add-game-path-input', value: props.value, placeholder: props.placeholder, onChange: changeCB }),
        React.createElement(react_bootstrap_1.InputGroup.Button, { className: 'inset-btn' },
            React.createElement(react_bootstrap_1.Button, { onClick: browseCB },
...
function InputAutosuggest(props) {
    const [focused, setFocus] = React.useState(false);
    const suggestions = focused && (props.value || '').length > 1
        ? (props.options || []).filter((opt) => opt.includes(props.value))
        : [];
    const hit = suggestions.length === 1 && suggestions[0] === props.value;
    return (React.createElement(React.Fragment, null,
        React.createElement(vortex_api_1.FormInput, { placeholder: props.placeholder, value: props.value, onFocus: delayed(setFocus), onChange: props.onChange }),
        suggestions.length === 0 || hit ? null : (React.createElement("div", { className: 'suggestions' }, suggestions.map((sugg) => (React.createElement(Suggestion, { key: sugg, value: sugg, onChange: props.onChange })))))));
}
function ModType(props) {
    const { t, spec, onRemove, onUpdate } = props;
    const setName = React.useCallback((name) => onUpdate({ ...spec, name }), [spec, onUpdate]);
    const setPath = React.useCallback((targetPath) => onUpdate({ ...spec, targetPath }), [spec, onUpdate]);
    const remove = React.useCallback(() => {
        onRemove(spec.id);
    }, [spec, onRemove]);
    return (React.createElement(react_bootstrap_1.ListGroupItem, { className: 'add-game-mod-type' },
        React.createElement(vortex_api_1.FlexLayout, { type: 'row', style: { alignItems: 'center' } },
            React.createElement(vortex_api_1.FlexLayout.Flex, null,
                React.createElement(FormLine, { t: t, controlId: 'add-game-modtype-name', title: t('Name') },
                    React.createElement(vortex_api_1.FormInput, { className: 'layout-flex', label: t('Name'), value: spec.name, onChange: setName })),
                React.createElement(FormPathItem, { t: t, label: t('Path'), value: spec.targetPath, onChange: setPath })),
            React.createElement(vortex_api_1.FlexLayout.Fixed, null,
                React.createElement(vortex_api_1.tooltip.IconButton, { icon: 'delete', tooltip: t('Remove'), onClick: remove })))));
}
function StopPattern(props) {
    const { t, pattern, onRemove, onUpdate } = props;
    const setValue = React.useCallback((value) => onUpdate({ ...pattern, value }), [pattern, onUpdate]);
    const remove = React.useCallback(() => {
        onRemove(pattern.id);
    }, [pattern, onRemove]);
    return (React.createElement(react_bootstrap_1.ListGroupItem, { className: 'add-game-stop-pattern' },
        React.createElement(vortex_api_1.FormInput, { className: 'layout-flex', label: t('Name'), value: pattern.value, onChange: setValue }),
        React.createElement(vortex_api_1.tooltip.IconButton, { icon: 'delete', tooltip: t('Remove'), onClick: remove })));
}
function nop() { }
const STEPS = [
...
class AddGameDialog extends vortex_api_1.ComponentEx {
    constructor(props) {
        super(props);
        this.mChangeFuncs = {};
        this.cancel = () => {
        ...
        this.save = async () => {
            ...
            const res = {
                game: {
                    ...
                    executable,
                    logo: imageName,
                    mergeMods: (_a = values['merge_mods']) !== null && _a !== void 0 ? _a : true,
            ...
            let modPathRelative = path.relative(values['game_path'], (_b = values['mod_path']) !== null && _b !== void 0 ? _b : values['game_path']);
            if (modPathRelative === '') {
                modPathRelative = '.';
            }
        ...
        this.addEmptyModType = () => {
            const emptyType = {
                id: `${this.deduceId()}-${shortid_1.generate()}`,
                name: 'New Mod Type',
                priority: 'high',
        ...
        this.removeModType = (typeId) => {
            const idx = this.state.values['mod_types'].findIndex(type => type.id === typeId);
            if (idx !== -1) {
                this.nextState.values['mod_types'].splice(idx, 1);
            }
        };
        this.updateModType = (newType) => {
            const { values } = this.state;
            const idx = values['mod_types'].findIndex((type) => type.id === newType.id);
            if (idx === -1) {
        ...
        this.removeStopPattern = (patternId) => {
        ...
        this.updateStopPattern = (pattern) => {
            const { values } = this.state;
            const idx = values['stop_patterns'].findIndex((iter) => iter.id === pattern.id);
            if (idx === -1) {
        ...
        this.validateName = () => {
            if ((this.state.values['name'] || '').length === 0) {
                return {
                    state: 'error',
                    reason: "Can't be empty. Please use the proper game name formatted " +
        ...
        this.validateDomain = () => {
            if ((this.state.values['nexus_domain'] || '').length === 0) {
                return {
                    state: 'warning',
                    reason: 'If this game exists on nexusmods.com, please enter the part ' +
        ...
        this.validateGamePath = () => {
            if ((this.state.values['game_path'] || '').length === 0) {
                return {
                    state: 'error',
                    reason: 'Please select the top-most folder of the game, meaning the ' +
        ...
        this.validateExePath = () => {
            const { values } = this.state;
            if ((values['exe_path'] || '').length === 0) {
                return {
                    state: 'error',
                    reason: "Can't be empty, please select the executable to start the " +
                        'game with',
                };
            }
            else if (path.relative(values['game_path'], values['exe_path']).startsWith('..')) {
                return {
                    state: 'error',
    ...
    render() {
        ...
        const validated = this.validate(step);
        return (React.createElement(vortex_api_1.Modal, { show: visible, onHide: nop, id: 'add-game-dialog' },
            React.createElement(vortex_api_1.Modal.Header, null,
                React.createElement(vortex_api_1.Modal.Title, null, t('Add a game')),
                this.renderStep(step)),
            React.createElement(vortex_api_1.Modal.Body, null, working ? this.renderWorking() : page()),
            React.createElement(vortex_api_1.Modal.Footer, null,
                React.createElement(react_bootstrap_1.Button, { onClick: this.cancel }, t('Cancel')),
                (step === 'intro') ? null : React.createElement(react_bootstrap_1.Button, { onClick: this.back }, t('Back')),
                step === STEPS[STEPS.length - 1] ? (React.createElement(react_bootstrap_1.Button, { disabled: !validated, onClick: this.save }, t('Save'))) : (React.createElement(react_bootstrap_1.Button, { disabled: !validated, onClick: this.next }, t('Next'))))));
    }
    renderStep(step) {
        const { t } = this.props;
        return (React.createElement(vortex_api_1.Steps, { step: step, style: { marginBottom: 32 } },
            React.createElement(vortex_api_1.Steps.Step, { key: 'intro', stepId: 'intro', title: t('Introduction'), description: t('Introduction') }),
            React.createElement(vortex_api_1.Steps.Step, { key: 'references', stepId: 'references', title: t('References'), description: t('The game on other platforms') }),
            React.createElement(vortex_api_1.Steps.Step, { key: 'info', stepId: 'info', title: t('Info'), description: t('Base game information') }),
            React.createElement(vortex_api_1.Steps.Step, { key: 'technical', stepId: 'technical', title: t('Technical'), description: t('Technical details') }),
            React.createElement(vortex_api_1.Steps.Step, { key: 'modtypes', stepId: 'modtypes', title: t('Mod Types'), description: t('Mod Types') }),
            React.createElement(vortex_api_1.Steps.Step, { key: 'review', stepId: 'review', title: t('Review'), description: t('Creation result') })));
    }
    renderWorking() {
    ...
    renderIntro() {
        const { t } = this.props;
        return (React.createElement("div", { style: { display: 'flex', height: '100%', justifyContent: 'center', alignItems: 'center' } },
    ...
    renderReferences() {
        ...
        Object.keys(storeGames).forEach((storeId) => {
            storeGames[storeId].forEach((gameEntry) => {
                if (gameEntry.name !== undefined) {
                    ...
                    vortex_api_1.log('warn', 'invalid game entry', JSON.stringify(gameEntry));
                }
            });
        });
        ...
        return (React.createElement(react_bootstrap_1.Form, { horizontal: true },
            React.createElement(react_bootstrap_1.HelpBlock, null,
                t('First, please select which game you want to add the catalog ' +
                    'of games supported on www.nexusmods.com and games installed ' +
                    'through launchers (Steam, ...) on your system.'),
                React.createElement("br", null),
                t('Both are optional but if they apply we can fill out much ' +
                    'of the information required automatically.')),
            React.createElement(FormLine, { t: t, controlId: 'add-game-nexusdomain', title: t('Nexus Domain'), validationFunc: this.validateDomain },
                React.createElement(InputAutosuggest, { options: nexusGames.map((game) => game.domain_name), placeholder: 'Nexus Domain', value: values['nexus_domain'], onChange: this.changeFunc('nexus_domain') })),
            React.createElement(FormLine, { t: t, controlId: 'add-game-storegames', title: t('Game') },
                React.createElement(react_select_1.default, { className: 'select-compact', options: options, value: values['store_game'] || '', onChange: this.changeFunc('store_game', (input) => input === null || input === void 0 ? void 0 : input.value) }))));
    }
    renderInfo() {
        const { t } = this.props;
        const { values } = this.state;
        return (React.createElement(react_bootstrap_1.Form, { horizontal: true },
            React.createElement(FormLine, { t: t, controlId: 'add-game-name', title: t('Name'), validationFunc: this.validateName },
                React.createElement(vortex_api_1.FormInput, { placeholder: 'Game Name', value: values['name'], onChange: this.changeFunc('name') })),
            React.createElement(FormLine, { t: t, controlId: 'game-image', title: t('Image') },
                React.createElement("img", { className: 'gameicon', src: values['image_url'] }),
                React.createElement(vortex_api_1.FormInput, { value: values['image_url'], onChange: this.changeFunc('image_url'), debounceTimer: 5000 })),
            React.createElement(react_bootstrap_1.HelpBlock, null, t('Please use an image in 16:9 format to fit into the grid ' +
                'inside Vortex.')),
            React.createElement(FormLine, { t: t, controlId: 'game-path', title: t('Game Path'), validationFunc: this.validateGamePath },
                React.createElement(PathInput, { api: this.context.api, value: values['game_path'], placeholder: t('Select path where the game is installed'), onChange: this.changeFunc('game_path') })),
            React.createElement(FormLine, { t: t, controlId: 'mod-path', title: t('Mod Path') },
                React.createElement(PathInput, { api: this.context.api, defaultpath: values['game_path'], value: values['mod_path'], placeholder: t('Select path where mods get installed'), onChange: this.changeFunc('mod_path') })),
            React.createElement(vortex_api_1.Usage, { infoId: 'add-game-mod-path', persistent: true },
                t('Some games require mods to be installed in different directories. ' +
                    'If this is the case, please select the most commonly used path, ' +
                    'you will be able to set up additional mod types later on. '),
                t('If you select nothing, mods get installed into the base ' +
                    'folder of the game.')),
            React.createElement(FormLine, { t: t, controlId: 'add-game-start-via-store', title: t('Start via Store') },
                React.createElement(vortex_api_1.Toggle, { disabled: values['store_game'] === undefined, checked: values['start_via_store'] !== undefined, onToggle: this.changeFunc('start_via_store') })),
            React.createElement(vortex_api_1.Usage, { infoId: 'add-game-start-via-store', persistent: true }, t('Some games can\'t be run directly but have to be run through the store ' +
                'you bought them on. Please enable only if that is necessary.')),
            React.createElement(FormLine, { t: t, controlId: 'exe-path', title: t('Executable'), validationFunc: this.validateExePath },
                React.createElement(PathInput, { api: this.context.api, defaultpath: values['game_path'], value: values['exe_path'], placeholder: t('Select the executable used to start this game'), onChange: this.changeFunc('exe_path'), selectFile: true, filters: [
    ...
    renderTechnical() {
        ...
        const { values } = this.state;
        return (React.createElement(react_bootstrap_1.Form, { horizontal: true },
            React.createElement(FormLine, { t: t, controlId: 'add-game-merge-mods', title: t('Merge Mods') },
                React.createElement(vortex_api_1.Toggle, { checked: (_a = values['merge_mods']) !== null && _a !== void 0 ? _a : true, onToggle: this.changeFunc('merge_mods') })),
            React.createElement(vortex_api_1.Usage, { infoId: 'add-game-merge', persistent: true },
                t('Most games expect all mods to be installed into a single directory. Tther times all mods ' +
            ...
            React.createElement(react_bootstrap_1.ControlLabel, null, t('Stop Pattern')),
            React.createElement(react_bootstrap_1.ListGroup, null,
                (values['stop_patterns'] || []).map((pattern) => (React.createElement(StopPattern, { t: t, key: pattern.id, pattern: pattern, onUpdate: this.updateStopPattern, onRemove: this.removeStopPattern }))),
                React.createElement(react_bootstrap_1.ListGroupItem, null,
                    React.createElement(react_bootstrap_1.Button, { onClick: this.addStopPattern }, t('Add Stop-pattern')))),
            React.createElement(vortex_api_1.Usage, { infoId: 'add-game-stoppattern', persistent: true },
    ...
    renderModType() {
        ...
        return (React.createElement("div", { style: { width: '100%', height: '100%' } },
            React.createElement(react_bootstrap_1.HelpBlock, null,
                t('Mod Types are for cases where the game has different types of mods that need to ' +
                ...
                React.createElement("br", null),
                t("For most game it's safe to leave this empty, please don't use mod types if you " +
            ...
            React.createElement(react_bootstrap_1.ListGroup, null,
                (values['mod_types'] || []).map((type) => (React.createElement(ModType, { t: t, key: type.id, spec: type, onUpdate: this.updateModType, onRemove: this.removeModType }))),
                React.createElement(react_bootstrap_1.ListGroupItem, null,
                    React.createElement(react_bootstrap_1.Button, { onClick: this.addEmptyModType }, t('Add Mod Type'))))));
    }
    renderReview() {
        const { t } = this.props;
        return (React.createElement("div", { style: {
            ...
            React.createElement("div", { style: { fontSize: 'larger' } }, t('All done. You will have to restart Vortex for the game to show up.'))));
    }
    deduceId() {
        const { values } = this.state;
        if (values['nexus_domain'] !== undefined) {
        ...
        return values['name'].toLowerCase().replace(/[ -_]/g, '');
    }
    onEnterInfo() {
        ...
        const nexusInfo = nexusGames.find((game) => game.domain_name === values['nexus_domain']);
        let storeInfo;
        if (values['store_game'] !== undefined) {
            const [storeId, appId] = values['store_game'].split(':');
            storeInfo = (storeGames[storeId] || []).find((entry) => entry.appid === appId);
        }
        ...
        if (nexusInfo !== undefined) {
            ...
            setIfEmpty('image_url', `https://staticdelivery.nexusmods.com/Images/games/cover_${nexusInfo.id}.jpg`);
        }
        return Promise.resolve();
    }
    changeFunc(key, mapper) {
        if (this.mChangeFuncs[key] === undefined) {
            this.mChangeFuncs[key] = (newValue) => {
...
exports.default = react_i18next_1.withTranslation(['add-game', 'default'])(AddGameDialog);
...
Object.defineProperty(exports, "__esModule", { value: true });
const AddGameDialog_1 = __webpack_require__(/*! ./AddGameDialog */ "./src/AddGameDialog.tsx");
const path = __webpack_require__(/*! path */ "path");
const template = __webpack_require__(/*! string-template */ "./node_modules/string-template/index.js");
const vortexApi = __webpack_require__(/*! vortex-api */ "vortex-api");
const vortex_api_1 = __webpack_require__(/*! vortex-api */ "vortex-api");
const { actions, fs, util } = vortexApi;
const GAME_STORES = ['steam', 'epic', 'origin', 'uplay', 'gog', 'xbox'];
const WIKI_URL = 'https://wiki.nexusmods.com/index.php/Packaging_extensions_for_Vortex';
...
function makeInfo(game, userName) {
    return {
        name: `Game: ${game.name}`,
        author: userName,
        version: '1.0.0',
...
async function exportGame(api, spec, imageUrl) {
    var _a, _b;
    try {
        const state = api.getState();
        spec.game.logo = path.basename(spec.game.logo);
        const baseExtCode = await fs.readFileAsync(path.join(__dirname, 'ext_template.js'), { encoding: 'utf8' });
        ...
        const userName = ((_b = (_a = state.persistent['nexus']) === null || _a === void 0 ? void 0 : _a.userInfo) === null || _b === void 0 ? void 0 : _b.name) || process.env.USERNAME;
        await fs.writeFileAsync(path.join(exportPath, 'info.json'), JSON.stringify(makeInfo(spec.game, userName), undefined, 2), { encoding: 'utf8' });
            ...
            actions: [
                {
                    title: 'Further steps',
                    action: () => {
                        api.showDialog('info', 'Further steps', {
                            bbcode: 'The game extension was created in [url="file://{{extensionPathUrl}}"]{{extensionPath}}[/url]<br/>' +
        ...
        Promise.all(GAME_STORES.map((storeId) => {
            try {
                ...
                return store
                    ...
                    .then((games) => ({
                    storeId,
                    games: util.unique(games, (item) => item.appid),
                }));
            }
            catch (err) {
                vortex_api_1.log('debug', 'game store not found', { storeId, error: err.message });
                return null;
            }
        })).then((results) => {
            results.forEach((res) => {
